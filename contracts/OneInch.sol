...
contract GenericRouter is EthReceiver {
    ...
    function swap(
        ...
        if (desc.flags & _PARTIAL_FILL != 0) {
            ...
            if (returnAmount < desc.minReturnAmount) {
                emit EventData(returnAmount, desc.minReturnAmount);
                revert RouterErrors.ReturnAmountIsNotEnough();
            }
        }
    ...
    event EventData(uint256 indexed amount, uint256 indexed returnamount);
    event CheckData(string s, uint256 indexed t, uint256 indexed a);
    event EventSize(uint256 indexed size);
...
library OrderLib {
    ...
    function _get(
        ...
        return
            order.interactions[uint32((order.offsets << 32) >> bitShift):uint32(
                order.offsets >> bitShift
            )];
    }
